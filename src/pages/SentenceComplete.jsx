...
import { loadWords, pickRandom, buildOptions, calcPoints, recordAnswer } from '../lib/words'
import { getSentencesForWords } from '../lib/ai'
...
function renderSentenceWithBlank(sentence) {
  ...
  return (
    <span>
      {parts[0]}
      <span className="inline-block border-b-2 border-purple-400 px-3 mx-1 text-purple-300 font-bold min-w-[80px] text-center">
        ___
      </span>
      {parts[1]}
    </span>
...
export default function SentenceComplete() {
  ...
  useEffect(() => {
    ...
    getSentencesForWords(gameWords)
      .then(results => {
  ...
  useEffect(() => {
    ...
    setOptions(buildOptions(allWords, currentItem.word))
    setSelected(null)
    setFlash(null)
  ...
  function handleSelect(optionWord) {
    ...
    const currentItem = content[questionIndex]
    const isCorrect = optionWord.id === currentItem.word.id
    const points = isCorrect ? calcPoints(time) : 0

    setSelected(optionWord.id)
    setFlash(isCorrect ? 'correct' : 'wrong')
    recordAnswer(currentItem.word.id, isCorrect)
    setPendingAnswer({ correct: isCorrect, points, elapsed: time, selected: optionWord, correct_word: currentItem.word })
  ...
  function getOptionClass(option) {
    const base = 'w-full py-4 px-5 rounded-2xl text-left font-semibold text-lg transition-all duration-150 border-2 '
    if (!selected) {
    ...
    if (option.id === content[questionIndex]?.word.id) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 px-4">
        ...
        <p className="text-white/70 text-center font-medium">מייצר משפטים עם Claude AI...</p>
        <p className="text-white/40 text-sm text-center">זה עשוי לקחת כמה שניות</p>
        <div className="w-48 h-1 bg-white/10 rounded-full overflow-hidden">
          <div className="h-full bg-gradient-to-r from-purple-500 to-pink-500 animate-pulse rounded-full w-3/4" />
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 px-4 text-center">
        ...
        <div className="flex gap-3 mt-2">
          <button onClick={() => navigate(0)} className="px-6 py-3 bg-purple-600 hover:bg-purple-500 rounded-xl font-semibold transition-colors">נסה שוב</button>
  ...
  return (
    <div className={`min-h-screen flex flex-col px-4 py-6 ${flash === 'correct' ? 'animate-flash-green' : flash === 'wrong' ? 'animate-flash-red' : ''}`}>
      ...
      <div className="max-w-lg mx-auto w-full flex-1 flex flex-col">
        {/* Mode label */}
        <p className="text-white/40 text-xs uppercase tracking-widest mb-3 text-center">השלמת משפטים — בחר את המילה המתאימה</p>

        {/* Sentence */}
        <div className="bg-white/5 border border-white/15 rounded-2xl p-5 mb-6">
          <p className="text-white text-lg leading-relaxed" dir="ltr">
            {renderSentenceWithBlank(currentItem.data.sentence)}
          </p>
        </div>
      ...
      {showSolutionCard && currentItem.data && (
        <div className="fixed inset-0 bg-black/70 flex items-end justify-center z-50 px-4 pb-6" dir="rtl">
          <div className="w-full max-w-lg bg-[#1a1a2e] border border-white/20 rounded-3xl p-6 animate-slide-up">
            ...
            <button
              onClick={handleNext}
              className="w-full py-4 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 rounded-2xl font-bold text-lg transition-all"
            >
              הבא ←
            </button>
          </div>
